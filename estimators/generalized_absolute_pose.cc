...
#undef CAMERA_MODEL_CASE
    ...
    problem.AddResidualBlock(cost_function, loss_function,
                             qvec_data, tvec_data,
                             rig_qvecs[camera_idx].data(),
                             rig_tvecs[camera_idx].data(),
...
py::dict seq_absolute_pose_refiner(
        const std::vector<std::vector<Eigen::Vector2d>> points2D,
        const std::vector<std::vector<Eigen::Vector3d>> points3D,
        const std::vector<Camera> cameras,
        std::vector<Eigen::Vector4d> rig_qvecs,
    ...
    success_dict["num_inliers"] = report.support.num_unique_inliers;
    success_dict["inliers"] = inliers;
    success_dict["seq_tvecs"] = py::array(py::cast(std::move(rig_tvecs)));
    success_dict["seq_qvecs"] = py::array(py::cast(std::move(rig_qvecs)));
...
void bind_generalized_absolute_pose_estimation(py::module& m) {
    ...
    m.def(
        "seq_absolute_pose_refiner",
        static_cast<py::dict (*)(const std::vector<std::vector<Eigen::Vector2d>> points2D,
                                const std::vector<std::vector<Eigen::Vector3d>> points3D,
                                const std::vector<Camera> cameras,
                                std::vector<Eigen::Vector4d> rig_qvecs,
                                std::vector<Eigen::Vector3d> rig_tvecs,
        ...
        py::arg("refinement_options") = ref_options,
        py::arg("return_covariance") = false,
        "Absolute pose refinement with non-linear refinement for a image sequence.");
}
